    ...
    <template #header>
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <p class="text-sm text-stone-400 pt-1">Remaining Space:   <span :style="{ color: percentageColor }">{{ productStore.formatSize(freeSpace.toFixed(1)) }}</span> / {{ productStore.formatSize(device.assignedStorage.limit) }} limit</p>
        </div>
      </div>
    </template>
