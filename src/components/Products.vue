    ...
    <template #header>
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <p class="text-sm text-stone-400 font-semibold pt-1">Remaining Space:   <span :style="{ color: percentageColor }" class="text-sm">{{ productStore.formatSize(freeSpace.toFixed(1)) }}</span></p>
        </div>
      </div>
    </template>
